...
# Distance matrix where each DistanceMatrix[i][j] represent distance between the ith and jth datapoint
...
def Averagelinkage(child, first, second, dataset):
    sum = 0
    for child1 in child[first]:
        for child2 in child[second]:
            sum += distance(dataset[child1], dataset[child2])
    return sum / (len(child[first]) * len(child[second]))
...
def computeMatrix(distanceMatrix, parent, child, closedClustere, dataset):
    for i in range(len(distanceMatrix)):
        if parent[i] == i:
            distanceMatrix[i][closedClustere[0]] =  Averagelinkage(child, closedClustere[0], i, dataset)
